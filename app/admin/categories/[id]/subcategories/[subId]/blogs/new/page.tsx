...
export default function NewSubcategoryBlogPage() {
  const router = useRouter();
  const params = useParams();
  const categoryId = parseInt(params.id as string, 10);
  const subcategoryId = parseInt(params.subId as string, 10);
    ...
    const fetchData = async () => {
      try {
        ...
        if (catData.success && catData.category) {
          ...
          const sub = catData.category.subcategories.find(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/newsblogs", {
        method: "POST",
        headers: {
    ...
    <div>
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
          New Blog {category && subcategory && `in ${category.name} → ${subcategory.name}`}
        </h1>
      </div>
